...
describe("Exchange", () => {
    ...
    describe("addLiquidity", async () => {
        it("adds liquidity", async () => {
            await token.approve(exchange.address, toWei(200));      // 向exchange合约地址中转入200代币
            await exchange.addLiquidity(toWei(200), {value: toWei(100)});   // 将200代币和100ethers都存入exchange合约地址中
    ...
    describe("getEthAmount", async () => {
        it("returns correct ether", async () => {
            await token.approve(exchange.address, toWei(2000));
            await exchange.addLiquidity(toWei(2000), { value: toWei(1000) });
